...
async def analyze_markdown_file(base_prompt_text: str,
    ...
    try:
        ...
        output_parent_path = output_directory_path / str(file_parent_path.relative_to(input_directory_path/"raw-markdown"/"HMN_Fall24"))
...
if __name__ == "__main__":
    ...
    asyncio.run(analyze_directory(base_directory=str(outer_base_directory),
                                  output_directory=str(Path(OUTPUT_DIRECTORY) / f"{in_server_name}-checkpoint-{checkpoint_name}-ai-processed"),
                                  json_schema_model=ExtractedTextData,
