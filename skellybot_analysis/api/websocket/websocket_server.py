...
class SkellybotAnalysisWebsocketServer:
    def __init__(self, websocket: WebSocket):
        self.websocket = websocket
        self._skellybot_analysis_app_state: SkellybotAnalysisAppState = get_skellybot_analysis_app_state()
        self.frontend_image_relay_task: Optional[asyncio.Task] = None

    async def __aenter__(self):
        logger.debug("Entering SkellybotAnalysis  WebsocketServer context manager...")
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        logger.debug(" SkellybotAnalysis  WebsocketServer context manager exiting...")
    ...
    async def _ipc_queue_relay(self):
        ...
        try:
            while True:
                if self._skellybot_analysis_app_state.ipc_queue.qsize() > 0:
                    try:
                        await self._handle_ipc_queue_message(
                            message=self._skellybot_analysis_app_state.ipc_queue.get())
